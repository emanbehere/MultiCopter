...
public class MultiWiiConf_2_0 extends PApplet {
...
public void setup() {
  ...
  confRC_RATE = controlP5.addNumberbox("RC RATE",1,xParam+40,yParam+213,30,14);confRC_RATE.setDecimalPrecision(2);confRC_RATE.setMultiplier(0.02f);confRC_RATE.setLabel("");
  ...
  confRC_EXPO = controlP5.addNumberbox("RC EXPO",0,xParam+40,yParam+240,30,14);confRC_EXPO.setDecimalPrecision(2);confRC_EXPO.setMultiplier(0.01f);confRC_EXPO.setLabel("");
...
public void READ() {
  ...
  for(int i=0;i<PIDITEMS;i++) {confP[i].setValue(byteP[i]/10.0f);confI[i].setValue(byteI[i]/1000.0f);confD[i].setValue(byteD[i]);}
  ...
  for(int i=0;i<CHECKBOXITEMS;i++) for(int a=0;a<6;a++) {
    if ((PApplet.parseByte(activation1[i])&(1<<a))>0) checkbox1[i].activate(a); else checkbox1[i].deactivate(a);
    if ((PApplet.parseByte(activation2[i])&(1<<a))>0) checkbox2[i].activate(a); else checkbox2[i].deactivate(a);
  }
...
public void WRITE() {
  ...
  int p = 0;
   ...
   for(int i =0;i<frame_size_write;i++)    g_serial.write(PApplet.parseChar(s[i]));
}
...
public void processSerialData() {
  ...
  if (g_serial.read() == 'M') {
    ...
    if (inBuf[frame_size_read-1] == 'M') {  // Multiwii @ arduino send all data to GUI
      ...
      for(int i=0;i<CHECKBOXITEMS;i++)  { // highest bit contains mwc state for this item xxx
        if ((PApplet.parseByte(activation2[i])&(1<<7))>0) buttonCheckbox[i].setColorBackground(green_); else buttonCheckbox[i].setColorBackground(red_);
      }
...
class cGraph {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#F0F0F0", "MultiWiiConf_2_0" });
  }
}
